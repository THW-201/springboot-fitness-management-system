...
const loadEquipmentList = async () => {
  try {
    ...
    if (res.data) {
      equipmentList.value = (res.data.records || []).filter((item: Equipment) => item.coachId === currentUserId.value)
...
const handleAdd = () => {
  ...
  isEdit.value = false
  imageFileList.value = []
  formData.coachId = currentUserId.value
